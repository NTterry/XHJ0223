...
SYS_STA takeup(void)
{
	...
	do
	{
		...
		ERR_BREAK;													/*Òì³£Ö±½ÓÌø³öÑ­»·  do while*/		
		sethnum = cm2num(sys_attr.s_sethighcm);						//Éè¶¨¸ß¶È£¬»»Ëã³É³ÝÊý
        protectT = sys_attr.s_sethighcm * 30 + 5000;       			//ÐÞ¸ÄBug   2018.11.9
...
Debug("autolihe %d",sys_stadata.clihe);
		...
		while(Enc_Get_CNT1() < sethnum)				//Éè¶¨¸ß¶È  ±È½ÏµÄ³ÝÊý
		{
			...
			if(sys_attr.s_zidong == MOD_ZTT2)
			{
				if(GetEncoderLen1Cm() > sys_attr.s_hprot)	/*³¬¹ý0.5Ã×£¬´¥·¢±£»¤ ¹¼ÌÐøÀ­´¸£¬ÖØÐÂ¶¨ÒåÀëºÏµãµÄ¸ß¶È 2019.10.18 Terry*/
				{
															/*¼Ç×¡µ±Ç°µÄÌá´¸¸ß¶È */
					Prtop.flg = 1;
...
SYS_STA starttaking(void)
{
	...
	{
		ctime = osKernelSysTick();
		noloadpw = sys_attr.s_pnull * 3 / 2 + 1;			//µÈ´ý1.5±¶µÄ¿ÕÔØ¹¦ÂÊ  £¿£¿£¿
...
//		CheckDir(&sys_stadata.m_high);								//¸Ä±ä¼ÆÊý·½ÏòÖµ		
		sethnum = cm2num(sys_attr.s_sethighcm);							//½«¸ß¶È»»Ëã³É³ÝÊý
		ctime = osKernelSysTick();
		while(Enc_Get_CNT1() < sethnum)					//Éè¶¨¸ß¶È
		{
			...
			/******************ÖÐÍ¾Áï´¸ÅÐ¶Ï**************************/
			intime = osKernelSysTick();
			while(Enc_Get_Speed() < 1) 		//ÖÐÍ¾·¢ÏÖÁï´¸
			{
			...
			/***************ÉÏÀ­³¬Ê±ÅÐ¶Ï***********/
			if(getmilsec(ctime) > (sys_attr.s_sethighcm * 30 + 5000))   		/*ÉÏÀ­³¬Ê±ÅÐ¶Ï*/         
			{
				status |= ERR_CS;
				break;
			}
...
void sysattr_init(uint16_t flg)
{
    ...
    sys_attr.s_cnt = 6;         /*º»ÍÁ´ÎÊý Terry 2019.5.21*/
	sys_attr.s_hprot = 150;		/*Ä¬ÈÏ¸ß¶È±£»¤ÉèÖÃ Terry 2019.7.6*/
	sys_attr.s_pset = 0;		/*Ð£ÑéÎÞÐ§ Terry 2019.7.9*/
	...
	sys_stadata.TTCHK = 1;
	sys_attr.s_zidong = MOD_FREE;		//ÉÏµçºó±£³Ö¿ÕÏÐÄ£Ê½
}
