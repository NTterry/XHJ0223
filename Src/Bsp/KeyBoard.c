...
{
    uint32_t i = 0; 
    uint32_t readKey = 0;              //Essential init
    
    //GPIO Cyclic scan
    for(i = 0; i < keys.keyTotolNum; i++)
    {
        if(HAL_GPIO_ReadPin((GPIO_TypeDef*)keyS->singleKey[i].keyPort,keyS->singleKey[i].keyGpio) == keyS->singleKey[i].keyValidSta)
        {
...
uint32_t readKeyValue(keysTypedef_t *keyS)
{
    ...
    static uint32_t keyLongCheck = 0;
    ...
    {
        keyCheck = 0;
        keyPress = getKey(keyS);	// GetKeyStatus
        switch (keyState)
        {
            case 0:
                if(keyPress != 0)
...
{
    static int32_t key_total = -1;

    keyTypedef_t singleKey;
    
    //Platform-defined GPIO
    singleKey.keyValidSta = keyValidSta;
